...
export async function POST(req: Request) {
  try {
    ...
    const { error: profileEnsureError } = await ensureProfile(supabase, user);
    if (profileEnsureError) {
      console.error("[lessons/complete] ensureProfile:", profileEnsureError);
      return NextResponse.json({ error: profileEnsureError }, { status: 500 });
    }
    ...
    if (completionError) {
      console.error("[lessons/complete] insert completion:", completionError);
    ...
    const { error: profileError } = await supabase
      ...
      .eq("id", user.id);
    if (profileError) {
      console.error("[lessons/complete] update profile:", profileError);
    ...
    console.error("[lessons/complete] uncaught:", e);
